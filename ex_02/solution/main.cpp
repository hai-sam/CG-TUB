...
class KDTree : public SpatialDataStructure {
    ...
    std::vector<std::size_t> collectInRadius2D(Point const& p, float radius) const {
        std::vector<std::size_t> indices;
...
Point evaluateBezier(const std::vector<Point>& controlPoints, int m, int n, float u, float v) {
...
Normal evaluateBezierNormal(const std::vector<Point>& controlPoints, int m, int n, float u, float v) {
...
void callback() {
        ...
        if (sds != nullptr) {
            ...
            for (int j = 0; j < numV; ++j) {
                ...
                for (int i = 0; i < numU; ++i) {
                    ...
                    surfVertices[j * numU + i] = evaluateBezier(controlPoints, gridM, gridN, u, v);
                    surfNormals[j * numU + i] = evaluateBezierNormal(controlPoints, gridM, gridN, u, v);
                }
            }
            ...
            auto surfMesh = polyscope::registerSurfaceMesh("Bezier Surface", surfVertices, surfFaces);
            surfMesh->addVertexVectorQuantity("normals", surfNormals);
            
            auto ctrlMesh = polyscope::registerPointCloud("Control Points", controlPoints);
            ctrlMesh->setPointRadius(0.01);
        }
    }
    ...
    if (ImGui::Button("Generate MLS Surface")) {
        if (sds != nullptr) {
            ...
            auto mlsMesh = polyscope::registerSurfaceMesh("MLS Surface", mlsVertices, mlsFaces);
